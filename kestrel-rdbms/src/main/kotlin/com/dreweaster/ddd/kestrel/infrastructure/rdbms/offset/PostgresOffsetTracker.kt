package com.dreweaster.ddd.kestrel.infrastructure.rdbms.offset
...
class PostgresOffsetTracker(private val database: Database) : OffsetTracker {
    ...
    override fun getOffset(offsetKey: String): Mono<out Offset> = database.withContext { ctx ->
        ctx.select("SELECT last_processed_offset FROM event_stream_offsets WHERE name = :name", { LastProcessedOffset(it["last_processed_offset"].long) as Offset }) {
            this["name"] = offsetKey
        }
